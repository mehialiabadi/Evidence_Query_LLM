...
from utils.SID_Set_description import collate_fn, CustomDataset
...
def parse_args(args):
    ...
    parser.add_argument("--cls_loss_weight", default=0.5, type=float)
    parser.add_argument("--mask_loss_weight", default=0.5, type=float)
...
def main(args):
    ...
    for token in required_tokens:
        assert token in tokenizer.get_vocab(), f"{token} token not found in tokenizer."
    ...
    for n, p in model.named_parameters():
        if any(
            [
                x in n
                for x in ["lm_head","mask_decoder","cls_head"]
            ]
        ):
    ...
    if args.eval_only:
        acc, giou, ciou = validate(val_loader, model_engine, 0, writer, args)
        exit()

    validation_epochs = [1,3,5]
    if args.local_rank == 0:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) in validation_epochs:
            ...
            if args.no_eval == False:
                ...
                cur_acc = acc if is_best_acc else None
...
def train(
    ...
    progress = ProgressMeter(
        ...
        [batch_time, losses, ce_losses,cls_losses, mask_bce_losses, mask_dice_losses,mask_losses],
