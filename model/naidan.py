...
from .gag import GroupedAttentionGate
...
class SidaMetaModel:
    ...
    def initialize_sida_modules(self, config):
        ...
        sam_dim = 256
        self.gag = GroupedAttentionGate(
            F_g=sam_dim,   # gating signal channels
            F_l=sam_dim,   # "local" features channels
            F_int=sam_dim // 2,
            kernel_size=1,  # start with 1 for stability
            groups=1
        )
        self.gag.train()
        for p in self.gag.parameters():
        ...
        if (cls_labels == 2).any():
            ...
            for i in range(len(enhanced_pred_embeddings)):
                ...
                # low_res_masks, iou_predictions = self.model.visual_model.mask_decoder(
                 ...
                 # Mehry  image_embeddings=image_embeddings[i].unsqueeze(0),
                img_i = image_embeddings[i].unsqueeze(0).to(dense_embeddings.dtype)  # [1,256,64,64]

# ✅ Gate image feats using dense prompt feats
                gated_img_i = self.model.gag(g=dense_embeddings, x=img_i)  # same shape as img_i

                low_res_masks, iou_predictions = self.model.visual_model.mask_decoder(
                    image_embeddings=gated_img_i,
...
#mehry
        ...
        if not inference and seg_token_mask.sum() == 0:  
            ...
            for p in itertools.chain(
                ...
                self.model.gag.parameters(),   # by mehry

                self.model.attention_layer.parameters()):
                dummy = dummy + p.sum() * 0.0      
            mask_loss = mask_loss + dummy 
        #put cls 0
        loss = self.mask_loss_weight * mask_loss + 0.0 * cls_loss
