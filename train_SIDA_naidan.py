...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        #"--version", default="/project/shih/nz85/SIDA-main/ck/SIDA-7B"
        "--version", default="/project/shih/nz85/SIDA-main/ck/SIDA-13B"
    )
...
def main(args):
    ...
    # 1) Freeze everything (this guarantees no random modules are trainable)
    ...
    # 3) Find decoder module automatically (don’t guess mask_decoder)
    decoder_module = None
    decoder_candidates = []

    for name, module in base.named_modules():
        ln = name.lower()
        # common patterns in SAM/seg models
    ...
    # pick best candidate: prefer 'mask_decoder'
    for name, module in base.named_modules():
    ...
    # fallback: any module ending with 'decoder'
    ...
    for n, p in model.named_parameters():
        if any(
            [
                ...
                for x in ["embed_tokens", "text_hidden_fcs","cls_head", "sida_fc1","attention_layer"]
            ]
        ):
            p.requires_grad = False

    # for p in model.get_model().mask_decoder.parameters():
    #     p.requires_grad = True
...
# self.visual_model.pro
    ...
    train_dataset = CustomDataset(
        ...
        seed=0,  # Image size for resizing
    ...
    # dan
    # -------------------------------------------------------------------------
    # 为什么会出现 CUDA_HOME / FusedAdam 报错？
    # ds_config 里 "optimizer": {"type": "AdamW", ...} 会让 DeepSpeed 使用 FusedAdam。
    # FusedAdam 是 DeepSpeed 的 CUDA 融合算子，需要：
    #   (1) 在运行时 JIT 编译 C++/CUDA 扩展，或加载已编译的 .so；
    #   (2) 编译/加载时 PyTorch 的 cpp_extension 会找 CUDA 路径，即环境变量 CUDA_HOME。
    # 若未设置 CUDA_HOME，或环境里没有可用的 CUDA 开发环境，就会报：
    #   OSError: CUDA_HOME environment variable is not set.
    # 解决办法二选一：(A) 设置 CUDA_HOME 并保证能编译；(B) 不用的 FusedAdam，改用 PyTorch 自带的 AdamW。
    # -------------------------------------------------------------------------
    #add print
    ...
    ds_config = {
        ...
        # dan
        # 原写法：由 DeepSpeed 根据 type 创建优化器，会创建 FusedAdam，需要 CUDA_HOME
        # "optimizer": {
        #     "type": "AdamW",
        #     "params": {
        #         "lr": args.lr,
        ...
        #     },
        # },
        # 改为不在这里指定 optimizer，下面用 PyTorch AdamW 显式传入，避免 FusedAdam
        "scheduler": {
            "type": "WarmupDecayLR",
            "params": {
    ...
    # dan
    # 原写法：只传 config，DeepSpeed 会按 config 里的 "optimizer" 创建 FusedAdam → 需要 CUDA_HOME
    ...
    #     training_data=None,
    # )
    # 新写法：显式传入 PyTorch AdamW，DeepSpeed 不再创建 FusedAdam，无需 CUDA_HOME / JIT 编译
    #add delete lora
